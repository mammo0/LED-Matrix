...
% from led_matrix.common.schedule import CronStructure
% from led_matrix.server.http_server import CRON_DICT
% from led_matrix.server.http_server import get_cron_selector_pattern
        ...
        <div id="animation_settings" class="card">
            <div class="card-header" id="animation_collapse_header">
                <a class="btn btn-link" data-toggle="collapse" href="#animation_collapse_body" role="button" aria-expanded="false" aria-controls="animation_collapse_body">
                    ...
                    <span>Change Settings: {{entry.animation_name.title()}}</span>
                </a>
            </div>
            <div id="animation_collapse_body" class="collapse" aria-labelledby="animation_collapse_header">
                <div class="card-body">
                    % include("animation/settings.tpl", animation_controller=animation_controllers[entry.animation_name], animation_settings=entry.animation_settings)
                </div>
            </div>
        </div>
                    ...
                    % for category in CRON_DICT:
                        <div class="form-group">
                            ...
                            <div id="cron_{{category}}_select_text_container" class="input-group">
                                <input id="cron_{{category}}_select_text" type="text" class="form-control" name="cron_{{category}}_select_value" placeholder="Any" value="{{xstr(getattr(entry.cron_structure, category))}}" aria-describedby="cron_{{category}}_select_btn" pattern="{{get_cron_selector_pattern(category)}}">
...
<script>
    ...
    window.addEventListener("load", function(){
        var category_names = [{{!",".join(["'%s'" % category for category in CRON_DICT])}}];
        var categories = {};
        ...
        btn_schedule_animation.onclick = function() {
            ...
            form_data["selected_animation_name"] = "{{entry.animation_name}}";

            // this selector and form is defined by 'animation/settings.tpl'
            let animation_form = document.getElementById("animation_settings_form_{{entry.animation_name}}");
